...
{
//	debug("******** MNP entering srun_allocate_abort, job_ptr->job_id=%d", job_ptr->job_id);
	if (job_ptr && job_ptr->alloc_resp_port && job_ptr->alloc_node
		&&  job_ptr->resp_host) {
		slurm_addr_t * addr;
...
extern void srun_job_complete (struct job_record *job_ptr)
{
	...
	ListIterator step_iterator;
	struct step_record *step_ptr;
	//debug("******** MNP entering srun_job_complete, job_ptr->job_id=%d", job_ptr->job_id);

	xassert(job_ptr);
	//debug("******** MNP in srun_job_complete, job_ptr->other_port=%d", job_ptr->other_port);
	//debug("******** MNP in srun_job_complete, job_ptr->alloc_node=%s", job_ptr->alloc_node);
	//debug("******** MNP in srun_job_complete, job_ptr->resp_host=%s", job_ptr->resp_host);
	if (job_ptr->other_port && job_ptr->alloc_node && job_ptr->resp_host) {
		addr = xmalloc(sizeof(struct sockaddr_in));
		slurm_set_addr(addr, job_ptr->other_port, job_ptr->resp_host);
		...
		msg_arg->job_id   = job_ptr->job_id;
		msg_arg->step_id  = NO_VAL;
		//debug("******** MNP in srun_job_complete, sending SRUN_JOB_COMPLETE msg");
		_srun_agent_launch(addr, job_ptr->alloc_node,
...
extern void srun_step_complete (struct step_record *step_ptr)
{
	...
	//debug("******** MNP entering srun_step_complete, step_ptr->job_ptr->job_id=%d", step_ptr->job_ptr->job_id);
	xassert(step_ptr);
